...
import { BookOpen, Quote, ArrowRight, Heart, Music, Coffee, Film, Lightbulb, Bookmark } from "lucide-react"
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-10 sm:mb-12">
          ...
          <div className="bg-white rounded-xl shadow-md p-5 sm:p-6 md:p-8 border border-slate-200">
            ...
            {/* Ajustar las tarjetas de libros para que sean pequeñas en móvil pero grandes en desktop */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5 sm:mb-6">
              {favoriteBooks.map((book, index) => (
                <div key={index} className="relative group">
                  <div className="aspect-[2/3] rounded-lg overflow-hidden shadow-md relative max-w-[120px] sm:max-w-full mx-auto">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 z-20">
                      <h4 className="text-white font-bold text-xs sm:text-sm leading-tight mb-0.5">{book.title}</h4>
                      <p className="text-white/80 text-[10px] sm:text-xs">{book.author}</p>
                    </div>
                    <div
                      className={`absolute top-2 right-2 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-br ${book.color} flex items-center justify-center text-white shadow-sm z-20`}
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                },
                {
                  ...
                  title: "Café",
                  text: "Geisha de Panamá, preparado en V60",
                },
                {
                  icon: <Film className="h-4 w-4 sm:h-5 sm:w-5" />,
                  title: "Cine",
                  text: "Películas de Kurosawa y Fellini",
                },
              ].map((item, index) => (
                <div
                  key={index}
